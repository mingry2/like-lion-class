package algorithm.algo1012.Min;

public class Min {
    // return ; maxVlue
    public int getMin(int[] arr){
        int minValue = arr[0]; // arr이 모두 음수인 경우 0으로 하면 0이 max이기 때문에 0으로 초기화하지 않고 이렇게 초기화.
        for(int i = 1; i < arr.length; i++){ // 위의 변경으로 초기화하여 값이 3이 되니까 arr[0]인 3은 스캔할필요없음
            if(minValue > arr[i]) { // 최소값 구하기
                minValue = arr[i];
            }
        }
        return minValue;
